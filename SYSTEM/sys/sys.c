...
void SYS_Parameter_Init(void)
{
	...
	if(systemset.saveflag!=0X0A)
		{
			sprintf((char*)systemset.DefUsName,"KLL-ESP8266");
			sprintf((char*)systemset.DefPawd,"12345678");
			sprintf((char*)systemset.UserName,"ERROR");
			sprintf((char*)systemset.Passwd,"ERROR");
			systemset.SysSpeedBit=0;
			systemset.Handnum=0;
			systemset.Carnum=0;
			systemset.usrpwdsfg=0;
