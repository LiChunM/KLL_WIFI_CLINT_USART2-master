...
void User_Command_Analysis(u8 *buf)
{
	...
	if(p1!=NULL)
		{
			mymemset(systemset.DefUsName,0,sizeof(systemset.DefUsName));
			Get_Str_Use(systemset.DefUsName,p1);
	...
	if(p1!=NULL)
		{
			mymemset(systemset.DefPawd,0,sizeof(systemset.DefPawd));
			Get_Str_Use(systemset.DefPawd,p1);
			sysset_save_para(&systemset);
			printf("+DefPawd %s\r\n",systemset.DefPawd);
		}
	...
	if(p1!=NULL)
		{
			...
			printf("UsName:%s\r\n",systemset.UserName);
			printf("Pawd:%s\r\n",systemset.Passwd);
			printf("SysSpeedBit %d\r\n",systemset.SysSpeedBit);
			printf("Carnum %d\r\n",systemset.Carnum);
			printf("Handnum %d\r\n",systemset.Handnum);
		}
